...
export function ApplicationHeader() {
  ...
  const [selectedEvent, setSelectedEvent] =
    useState<TelemetryEventRow | null>(null)
  const telemetry =
    loadState.status === 'ready' ? loadState.telemetry : null
  const batteryIds = telemetry?.snapshot.batteryIds ?? []
  const eventRows =
    loadState.status === 'ready' ? loadState.batteryFleetData.eventRows : []
  ...
  if (loadState.status === 'error') {
    ...
    telemetryStatus =
      attentionCount > 0
        ? `${batteryCount} batteries · ${attentionCount} need review · ${eventCount} events`
        : `${batteryCount} batteries monitored · ${eventCount} events`
    telemetryStatusClassName =
      attentionCount > 0
      ...
      <div className="mx-auto w-full max-w-7xl px-4 md:px-6">
        <div className="grid min-h-16 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-2 gap-y-2 py-2 md:h-16 md:gap-3 md:py-0">
          <div className="flex items-center gap-2">
                  ...
                  <NavigationMenuList className="flex-col items-start gap-0">
                    {navigationLinks.map((link) => (
                      <NavigationMenuItem className="w-full" key={link.to}>
                        <NavigationMenuLink
                          active={isActiveRoute(pathname, link.to)}
                          asChild
                          className="py-1.5"
                        >
            ...
            <Link
              aria-label="Battery telemetry overview"
          ...
          <div
            className="order-3 col-span-3 mx-auto w-full max-w-none md:order-none md:col-span-1 md:max-w-sm"
            role="search"
          >
            <TelemetrySearch
              ...
              onBatterySelect={(batteryId) =>
                void navigate(`/batteries/${encodeURIComponent(batteryId)}`)
              }
        ...
        <div className="flex min-h-10 items-center justify-end gap-4 border-t py-1.5 md:justify-between">
          ...
          >
            <span
              aria-hidden="true"
              className={`size-2 shrink-0 rounded-full ${telemetryStatusClassName}`}
            />
            <span className="truncate">{telemetryStatus}</span>
          </div>
        </div>
      </div>
